...
6. A successful calculation opens a separate, zoomable Route page. It contains route controls, a summary, a detailed step table, and a Save map action. Back navigation returns to Edit map without discarding the calibration line or rotation.

## Saved maps

Saving writes the rectified bitmap as a PNG under the app's private files directory and stores its path, timestamp, pixels-per-metre scale, and current edited points in Room. The database and point rows are inserted transactionally; a failed database write removes the just-created image file. Recent maps observes Room reactively and reopens a selection directly into the completed Edit map state, retaining its stored scale for route calculation.
...
- Compact rotate, recenter, line-reset, and point-add HUD actions sit on an opaque Material surface, while canvases are clipped below the top bar.
...
- Rotation is shared by the map workflow. Labels are drawn in screen space so their text stays upright while the bitmap rotates.
...
The project contains JVM tests for preferences, distance matrices, routing, and OCR helpers, plus Android instrumentation tests for Room/repository persistence, image processing, ViewModels, Compose canvases, and combined start/finish route details. Standard verification is:
